...
◯　使用方法
　このコネクタはOpenCTI 6.6.9互換で、設定ファイル（config.yml）または環境変数で設定可能です。
...
　1. Dockerfile：コネクタの実行環境を構築するためのDockerfile:
...
　2. docker-compose.yml：OpenCTI環境に統合するための設定:
...
3. config.yml.sample：環境変数の代わりに使用できる設定ファイルのサンプル:
...
4. entrypoint.sh：コンテナ起動時の初期化スクリプト:
...
5. requirements.txt：必要なPythonライブラリのリスト:
